...
// /api/gmail/sync and /api/insights/generate are here too - both authenticate themselves (a
// real user session for a fresh call, a shared secret for server-to-server calls - see those
// routes), so this pre-filter would otherwise redirect their secret-authenticated calls (which
// carry no user cookie at all, e.g. Vercel Cron invoking /api/insights/generate) to /login.
const PUBLIC_ROUTES = ['/login', '/auth/callback', '/unauthorized', '/api/gmail/sync', '/api/insights/generate'];
